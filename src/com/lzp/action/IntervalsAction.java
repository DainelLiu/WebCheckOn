...
public class IntervalsAction {
	...
	public String list() throws IOException{
		...
		List<Object> intervalTypelist = intervalDao.list();//获取所有类型数据，不带分页
		PageBean page=null;
		if(intervalTypelist.size()>0){
			page = new PageBean(intervalTypelist.size(),pageNum,5);
			list = intervalDao.listAll(page);//带分页
		}
		JSONObject jobj = new JSONObject();
		if(intervalTypelist.size() > 0){
			//save success
	...
	public String listAll() throws IOException{
		...
		if(intervalTypelist.size() > 0){
			...
			jobj.put("mes", "获取成功!");
			jobj.put("status", "success");
			jobj.put("data", JsonUtil.toJsonByListObj(intervalTypelist));
		}else{
			//save failed
			jobj.put("mes", "获取失败!");
			jobj.put("status", "error");
