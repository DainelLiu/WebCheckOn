...
public class AbsenceAction {
	...
	public String delete() throws IOException{
		...
		Absence absence = absenceDao.getById(aId);
		JSONObject jobj = new JSONObject();
		if(absenceDao.delete(absence)){
			//save success
			jobj.put("mes", "删除成功!");
			jobj.put("status", "success");
		}else{
	...
	public String list() throws IOException{
		...
		List<Object> absenceTypelist = absenceDao.list();//获取所有类型数据，不带分页
		PageBean page=null;
		if(absenceTypelist.size()>0){
			page = new PageBean(absenceTypelist.size(),pageNum,5);
			list = absenceDao.listAll(page);//带分页
		}
		JSONObject jobj = new JSONObject();
		if(absenceTypelist.size() > 0){
			//save success
	...
	public String listAll() throws IOException{
		...
		if(absenceTypelist.size() > 0){
			...
			jobj.put("mes", "获取成功!");
			jobj.put("status", "success");
			jobj.put("data", JsonUtil.toJsonByListObj(absenceTypelist));
		}else{
			//save failed
			jobj.put("mes", "获取失败!");
			jobj.put("status", "error");
